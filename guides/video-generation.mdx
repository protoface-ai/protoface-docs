...
description: "Generate a video with an asynchronous Protoface run."
---

Choose a video model, submit a run, and retrieve the result when it finishes.
The API returns immediately after the run is queued.

<Note>
  [Protoface Studio](https://app.protoface.com) is the browser-based video
  editor. The API described here runs video models directly; it does not expose
  Studio projects or scenes.
</Note>
...
A completed run with one video output includes a `video` field:
