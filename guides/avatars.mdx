---
title: "Avatars"
description: "Create and use avatars in realtime sessions and avatar videos."
---

Use an avatar ID with realtime sessions and `POST /v1/videos`. Start with
`av_stock_001`, then switch to a custom avatar from the dashboard or API.

## Create an avatar

Custom avatar builds are asynchronous. Upload a portrait image, then poll the
avatar until `status` is `ready` or `failed`.

```sh
curl -X POST https://api.protoface.com/v1/avatars \
  -H "Authorization: Bearer $PROTOFACE_API_KEY" \
  -F "image=@portrait.png" \
  -F "name=Demo Avatar"
```

## Source image checklist

- Use a PNG or JPEG source image.
- Use one clearly visible, forward-facing face or face-like subject.
- Avoid heavy occlusion, extreme crop, harsh lighting, multiple faces, or
  screen-only/abstract faces without distinct features.
- See [Source Images](/guides/avatar-source-images) for examples of
  good and bad portrait uploads.
- If the avatar reaches `status: "failed"`, check `failure_reason` on the
  avatar response before uploading a new source image.

## List avatars

```sh
curl https://api.protoface.com/v1/avatars \
  -H "Authorization: Bearer $PROTOFACE_API_KEY"
```

Use avatars with `status: "ready"` in sessions or avatar video requests.

## Retrieve an avatar

```sh
curl https://api.protoface.com/v1/avatars/av_... \
  -H "Authorization: Bearer $PROTOFACE_API_KEY"
```

## Use an avatar

```python
from livekit.plugins import protoface

avatar = protoface.AvatarSession(avatar_id="av_stock_001")
```

When using the REST API, pass the same value as `avatar_id` in
`POST /v1/sessions`.

## Generate videos from audio

If you already have an audio file and want a finished avatar video, use
`POST /v1/videos`. It waits for the video generation to finish, then returns MP4 and HLS
URLs. It works best with clear speech, such as TTS output or a clean voice
recording.

```sh
AUDIO_BASE64="$(base64 < speech.wav | tr -d '\n')"

curl -X POST https://api.protoface.com/v1/videos \
  -H "Authorization: Bearer $PROTOFACE_API_KEY" \
  -H "Content-Type: application/json" \
  -H "Idempotency-Key: video-demo-001" \
  -d "{
    \"avatar_id\": \"av_stock_001\",
    \"audio_base64\": \"$AUDIO_BASE64\",
    \"audio_format\": \"wav\",
    \"audio_sample_rate\": 16000,
    \"audio_channel_count\": 1
  }"
```

See [Avatar Videos](/guides/videos) for request fields, audio format guidance,
and retry behavior.

## Delete an avatar

```sh
curl -X DELETE https://api.protoface.com/v1/avatars/av_... \
  -H "Authorization: Bearer $PROTOFACE_API_KEY"
```

Deleting a custom avatar removes its uploaded source image and built assets.
Past session and usage records remain. Stock avatars cannot be deleted.

## Next

<Columns cols={3}>
  <Card title="Realtime quickstart" href="/quickstart" icon="rocket">
    Run a LiveKit Agents app with a Protoface avatar.
  </Card>
  <Card title="Embeds" href="/guides/embeds" icon="panel-top">
    Add an avatar to a website or share a standalone conversation link.
  </Card>
  <Card title="Avatar videos" href="/guides/videos" icon="video">
    Generate an avatar video from prerecorded audio.
  </Card>
</Columns>
