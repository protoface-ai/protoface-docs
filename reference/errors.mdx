...
If a run submission returns `429` with `error.type: "rate_limit"`, follow
`Retry-After` when it is present; otherwise back off before retrying. If it
returns `error.type: "quota_exceeded"`, wait for an active run to finish.
Monitor queued runs through the status endpoint instead of resubmitting them.

For endpoints that accept `Idempotency-Key`, reuse the key only when retrying
the same request. If anything changes, use a new key.

## Next

<Columns cols={2}>
  <Card title="Limits and credits" href="/reference/quotas" icon="gauge">
    Realtime limits, inference concurrency, and credit behavior.
  </Card>
